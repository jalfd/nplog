...
namespace np::log {
  struct NPLOG_EXPORT MessageBuffer {
    ...
    MessageBuffer(MessageBuffer&& other) noexcept
      : buf_begin(other.buf_begin), buf_end(other.buf_end), msg_end(other.msg_end) {
    ...
    std::string_view contents() const noexcept {
      if (buf_begin == nullptr) { return reinterpret_cast<const char*>(&buf_begin); }
      *msg_end = '\0';
      return std::string_view{buf_begin, messageSize()};
    };

    void clear() noexcept { msg_end = buf_begin; }
  ...
  private:
    ...
    void reserveAdditional(size_t additional) noexcept { reserve(additional + messageSize()); }
